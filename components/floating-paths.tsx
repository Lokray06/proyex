...
function FloatingPaths({ position }: { position: number }) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      <svg
        // ⭐️ ADDED transition-colors and duration for smooth theme change on the paths
        className="w-full h-full text-neutral-950 dark:text-white transition-colors duration-500"
...
export function FloatingPathsBackground({ className }: FloatingPathsBackgroundProps) {
  return (
    <div className={cn(
      ...
      // ⭐️ ADDED transition-colors and duration to the container background
